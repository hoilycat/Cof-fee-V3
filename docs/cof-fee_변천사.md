...
### 탄생 배경
커피 마시는 시간이 다음 날 컨디션에 얼마나 영향을 주는지 직접 계산하고 싶었던 것이 출발점. Python 스크립트 두 개(`main.py`, `logic.py`)와 Streamlit으로 최소한의 UI를 만들었습니다.
...
핵심 알고리즘은 v1에서 완성되었습니다. 이후 버전들이 모두 이 로직을 물려받습니다.
...
v1의 Python/Streamlit을 통째로 버리고 **React + TypeScript + Vite** 스택으로 처음부터 다시 썼습니다. 단순 계산기에서 멀티페이지 앱으로의 도약.
...
v2의 README에는 야심 찬 AI 아키텍처가 담겨 있습니다 — Neo4j 지식 그래프, ChromaDB 벡터 DB, LangChain + Tavily API를 통한 Agentic GraphRAG. 하지만 Phase 2로 분류되어 실제 코드에는 없습니다. v3가 이것을 실현합니다.

---

## v3 — AI가 실제로 들어오다 (YIE 통합)

> **"논문이 말합니다"** — 계획으로만 있던 AI 엔진을 실제로 연결

### v3의 시작

v2 레포를 그대로 클론해서 출발했습니다 (첫 커밋: `"chore: init cof-fee-v3 (cloned from v2, YIE integration branch)"`). 즉 v2의 모든 기능을 이어받고 AI 연동에 집중.

### 가장 큰 변화: YIE 실제 연동

v2에서 기획만 했던 AI 엔진이 v3에서 **YIE(Universal Insight Engine)** 라는 이름으로 실제 연결됐습니다.
...
`src/lib/yieClient.ts`가 신규 추가됐습니다. Stats 화면에서 최근 7일 평균 카페인 섭취량과 잔존량을 YIE에 보내면, 관련 논문 청크를 검색해 한국어 인사이트 카드로 돌려줍니다.

### AI 코치 콩이 (CoachChat 페이지)

v3 최대 신기능. `pages/CoachChat/` 페이지가 통째로 추가됐습니다.
...
v1, v2에는 없었던 **자동화 테스트**가 v3에서 처음 생겼습니다.
...
v2에 있던 GIF 에셋(`public/assets/gifs/` — 은하수 전환, 카페인 타임라인 애니메이션 webp 파일들)이 v3에서 제거됐습니다. 방향이 '시각적 화려함'보다 'AI 실용성'으로 이동했습니다.
...
Cof/fee는 "커피를 마신 시간을 기록하면 어떨까"라는 단순한 아이디어에서 출발해, 세 번의 전면 재설계를 거쳤습니다. v1은 아이디어를 증명했고, v2는 제대로 된 앱의 형태를 만들었으며, v3는 AI를 실제로 붙이는 단계입니다.

핵심 알고리즘(반감기 × 수면 예측 × 두통 예보)은 v1부터 지금까지 변하지 않았습니다. 껍데기가 바뀐 게 아니라, 그 위에 점점 더 많은 것이 쌓였습니다.
